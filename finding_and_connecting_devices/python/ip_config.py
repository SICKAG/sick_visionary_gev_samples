...
# subnet. Can be used only if the camera is not opened by any application.
...
# procedure open each subsequent camera boot or cable re-connect. To be able to apply the persistent settings,
...
def main():
    ...
    with Harvester() as h:
        ...
        if args.action == "list":
            ...
            force_ip(h, device_sn=args.device_sn, automatic=False,
                     ip=args.ip, subnet=args.subnet, gateway=args.gateway)
            ...
            persist_settings(h, device_sn=args.device_sn, persistent_ip=True,
                             dhcp=False, ip=args.ip, subnet=args.subnet, gateway=args.gateway)
        elif args.action == "persistent_current":
            persist_settings(h, device_sn=args.device_sn,
                             persistent_ip=True, dhcp=False, ip=None)
        elif args.action == "persistent_dhcp":
            persist_settings(h, device_sn=args.device_sn,
                             persistent_ip=False, dhcp=True)
        elif args.action == "persistent_lla":
            persist_settings(h, device_sn=args.device_sn,
                             persistent_ip=False, dhcp=False)
        else:
            sys.exit("ERROR: unexpected script action requested")

# Helper defining the script command line interface and parsing the arguments, returning result to the caller


def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Script to GigE Vision device IP configuration (current and/or persistent IP settings)")

    # Enable output of version information
    parser.add_argument('-v', '--version', action='version',
                        version=f'%(prog)s {__version__}')

    # Split the user interface using (mandatory) subparsers representing the individual supported actions
    subparsers = parser.add_subparsers(
        dest="action", help="Actions supported by the script (each with own specific arguments and help text)", required=True)
    ...
    def add_device_select_args(subparser):
        subparser.add_argument("-d", "--device_sn", required=True,
                               help="Serial number of the device to configure")

    def add_ip_address_args(subparser):
        subparser.add_argument("-i", "--IP", dest="ip",
                               required=True, help="Desired IP address")
        subparser.add_argument(
            "-s", "--subnet", required=True, help="Desired subnet mask")
        subparser.add_argument("-g", "--gateway", default="0.0.0.0",
                               help="Desired default gateway (zero if not provided)")
    ...
    list_parser = subparsers.add_parser(
        "list", description="Lists all devices currently visible in the system")

    # ... the force_ip option requires argument(s) for device selection and IP specification
    force_ip_parser = subparsers.add_parser(
        "force_ip", description="Forces the specified temporary IP/subnet settings to the selected device (which must not be in use during the operation)")
    ...
    force_auto_parser = subparsers.add_parser(
        "force_auto", description="Forces a suitable automatic temporary IP/subnet settings to the selected device (which must not be in use during the operation)")
    ...
    force_restart_parser = subparsers.add_parser(
        "force_restart", description="Forces restart of the IP configuration procedure of the selected device based on its current persistent IP configuration (the device must not be in use during the operation)")
    ...
    persistent_ip_parser = subparsers.add_parser(
        "persistent_ip", description="Configures the selected device to use the specified IP/subnet settings upon following boot procedures (the device must be in ready-to-open state)")
    ...
    persistent_current_parser = subparsers.add_parser(
        "persistent_current", description="Configures the selected device to use its current IP/subnet settings upon following boot procedures (the device must be in ready-to-open state)")
    add_device_select_args(persistent_current_parser)

    # ... the persistent_dhcp option requires argument(s) for device selection
    persistent_dhcp_parser = subparsers.add_parser(
        "persistent_dhcp", description="Configures the selected device to use DHCP protocol (with LLA fallback) to obtain IP/subnet settings upon following boot procedures (the device must be in ready-to-open state)")
    add_device_select_args(persistent_dhcp_parser)

    # ... the persistent_lla option requires argument(s) for device selection
    persistent_lla_parser = subparsers.add_parser(
        "persistent_lla", description="Configures the selected device to use LLA protocol to obtain IP/subnet settings upon following boot procedures (the device must be in ready-to-open state)")
...
def list_devices(h):
    print("Listing currently discovered devices, device count: {}".format(
    ...
    for device_info in h.device_info_list:
        ...
        print("\tSerial number: {} (identifies the device in other actions of this script)".format(
        ...
        for i in range(itf_node_map.DeviceSelector.max + 1):
            ...
            if device_info.id_ == itf_node_map.DeviceID.value:
                ...
                print("\tMAC address: {}".format(
                    itf_node_map.GevDeviceMACAddress.to_string()))
                print("\tIP address: {}".format(
                    itf_node_map.GevDeviceIPAddress.to_string()))
                print("\tSubnet mask: {}".format(
                    itf_node_map.GevDeviceSubnetMask.to_string()))
                print("\tDefault gateway: {}".format(
                    itf_node_map.GevDeviceGateway.to_string()))
                print("\tDiscovered on interface: {}/{}".format(itf_node_map.GevInterfaceSubnetIPAddress.to_string(),
                      itf_node_map.GevInterfaceSubnetMask.to_string()))
                break
        ...
        if device_info.access_status == DEVICE_ACCESS_STATUS_LIST.DEVICE_ACCESS_STATUS_READWRITE:
            try:
                with h.create(device_info) as ia:
                    ...
                    print("\tEnabled IP configuration protocols: persistent IP - {}, DHCP - {}, LLA - always True".format(
                        nm.GevCurrentIPConfigurationPersistentIP.value, nm.GevCurrentIPConfigurationDHCP.value))
                    if nm.GevCurrentIPConfigurationPersistentIP.value:
                        print("\tPersistent IP address/subnet/gateway: {}/{}/{}".format(nm.GevPersistentIPAddress.to_string(
                        ), nm.GevPersistentSubnetMask.to_string(), nm.GevPersistentDefaultGateway.to_string()))
            except Exception:
                print("(failed to query the persistent IP settings of the device")
        else:
            print(
                "\t(persistent IP settings can only be reported for fully accessible devices)")
        ...
        print("\tAccess status: {}".format(
            get_access_status_info(device_info.access_status)))
...
def force_ip(h, device_sn, automatic, ip=None, subnet=None, gateway=None):
    print("Forcing temporary IP configuration to the device with serial number:: {}".format(device_sn))
    ...
    for device_info in h.device_info_list:
        ...
        sys.exit(
            "The required device (serial number {}) was not found".format(device_sn))

    # The force IP requests will be ignored by the device if it is currently open by some other application.
    ...
    # To be able to force new IP configuration to using the GenTL Producer, we need to identify the device
    ...
    for i in range(itf_node_map.DeviceSelector.max + 1):
        ...
        sys.exit(
            "Failed to identify the device within GenTL Producer nodemap (unexpected failure)")
    ...
    # Protocol-wise the special case with IP==0 is the same as a "regular" FORCEIP, but the operation which happens
    ...
    if ip_restart_request:
        ...
        print("\tIP address: {}".format(
            itf_node_map.GevDeviceForceIPAddress.to_string()))
        print("\tSubnet mask: {}".format(
            itf_node_map.GevDeviceForceSubnetMask.to_string()))
        print("\tDefault gateway: {}".format(
            itf_node_map.GevDeviceForceGateway.to_string()))
...
def persist_settings(h, device_sn, persistent_ip, dhcp, ip=None, subnet=None, gateway=None):
    print("Writing persistent IP configuration to the device with serial number:: {}".format(device_sn))
    ...
    for device_info in h.device_info_list:
        if device_info.serial_number == device_sn:
            ...
            if device_info.access_status != DEVICE_ACCESS_STATUS_LIST.DEVICE_ACCESS_STATUS_READWRITE:
                sys.exit(
                    "The required device does not currently have READWRITE access status and cannot thus be configured (device list comman can provide further details")
            break
    else:
        sys.exit(
            "The required device (serial number {}) was not found".format(device_sn))
    ...
    with h.create({'serial_number': device_sn, 'access_status': DEVICE_ACCESS_STATUS_LIST.DEVICE_ACCESS_STATUS_READWRITE}) as ia:
        ...
        print("\tPersistent IP protocol active: {}".format(
            nm.GevCurrentIPConfigurationPersistentIP.value))
        print("\tDHCP protocol active: {}".format(
            nm.GevCurrentIPConfigurationDHCP.value))
        print("\tLLA protocol active: always active")
        if persistent_ip:
            print("\tPersistent IP address: {}".format(
                nm.GevPersistentIPAddress.to_string()))
            print("\tPersistent subnet mask: {}".format(
                nm.GevPersistentSubnetMask.to_string()))
            print("\tPersistent default gateway: {}".format(
                nm.GevPersistentDefaultGateway.to_string()))
...
def validate_ip_settings(ip, subnet, gateway, zero_ip_valid):
    ...
    try:
        ...
        sys.exit(
            "Please revise input parameters - this is not a valid IP address: {}".format(ip))
    ...
    try:
        ...
        sys.exit(
            "Please revise input parameters - this is not a valid subnet mask address: {}".format(subnet))
    ...
    if gateway is not None:
        ...
        try:
            ...
            sys.exit(
                "Please revise input parameters - this is not a valid gateway address: {}".format(gateway))
    ...
    try:
        ...
        sys.exit(
            "Please revise input parameters - this is not a valid IP/subnet combination: {}".format(interface_string))
    ...
    if int(ipv4_ip) == 0:
        if zero_ip_valid:
            ...
            sys.exit(
                "Please revise input parameters - zero is not a valid IP address in given context: {}".format(ip))
    ...
    if gateway is not None and int(ipv4_gateway) != 0:
        ...
        if not ipv4_gateway in ipv4_network:
            sys.exit(
                "Please revise input parameters - the gateway address {} does not belong to network {}".format(gateway, interface_string))
