...
def apply_param(nodeMapObj, paramName, paramValue):
    ...
    if not isinstance(nodeMapObj, NodeMap):
        raise TypeError(
            f"nodeMapObj argument must have type 'NodeMap' but is: {type(nodeMapObj)}")
    if not isinstance(paramName, str):
        raise TypeError(
            f"paramName argument must have type 'str' but is: {type(paramName)}")
    if not isinstance(paramValue, (str, int, bool, float)):
        raise TypeError(
            f"paramValue argument supported types 'str, int, bool' but is: {type(paramValue)}")
    ...
    if isinstance(paramValue, str):
        ...
        command = "nodeMapObj.{}.value = {}".format(
            paramName, np.float32(paramValue))
    ...
    try:
        ...
        raise AttributeError(
            f"Failed to apply node/value {paramName} = {paramValue}") from err
...
def set_components(nodeMapObj, selectedComponents):
  ...
  """
    ...
    if not isinstance(selectedComponents, list):
        raise TypeError(
            f"selectedComponents argument must have list-type but is: {type(selectedComponents)}")
    if len(selectedComponents) < 1:
        raise RuntimeError(
            f"selectedComponents argument must have min length of 1 but has {len(selectedComponents)}")
    for component in selectedComponents:
        if not isinstance(component, str):
            raise TypeError(
                f"All components in selectedComponents must be of type str but found: {type(component)}")
        if not is_available(nodeMapObj.ComponentSelector.get_entry_by_name(component)):
            raise RuntimeError(
                f"Component \'{component}\' not available on this device")
    ...
    for ele in nodeMapObj.ComponentSelector.entries:
        if is_available(ele):
            ...
            nodeMapObj.ComponentEnable.value = ele.symbolic in selectedComponents
