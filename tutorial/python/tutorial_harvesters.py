...
import numpy
from common import download_cti_file, get_cti_path, validate_setup
...
def main():
    ...
    with Harvester() as h:
        ...
        print("Discovered {} device(s), going to use the first entry in the test:".format(
        ...
        with h.create() as ia:
            ...
            print("Version of GenTL producer driver: {} ({})".format(
                ia.system.node_map.TLVersion.to_string(), ia.system.node_map.TLVendorName.to_string()))
            ...
            if enum_entry_implemented(nm, "ComponentSelector", "ImuBasic"):
            ...
            # need to check if given GenICam feature is implemented before accessing it (to keep this generic for all camera models).
            # Note that you could use similar checks also for other feature-access-mode checks such as whether given feature
            # is currently readable or writable - might come handy when accessing features depending on current camera state.
            if feature_implemented(nm, "ExposureAuto"):
                ...
                # discussed below.
                if feature_implemented(nm, "ExposureTimeSelector"):
                    nm.ExposureTimeSelector.value = 'Range'
            ...
            # we attempt below to set few more parameters, trying to archive reasonable output quality in such conditions.
            ...
            # possible validation filter level to get rid of eliminate most unwanted outliers (in the range data). The actual
            # suitable exposure time value strongly depends on your working conditions and the camera model, please adjust it
            # as needed when getting poor output results.
            # One more camera specific note: if currently tested camera models supports configuring the exposure time separately
            # for the range and intensity image sensor, we should specify which one we mean.
            # Note once again the checks for availability of features which are implemented only in a subset of the Visionary
            # camera models. When targeting a concrete model with known set of supported features, you could skip those checks.
            if feature_implemented(nm, "ExposureTimeSelector"):
                nm.ExposureTimeSelector.value = 'Range'
            if feature_implemented(nm, "MultiSlopeMode"):
                nm.MultiSlopeMode.value = 'Off'
            if feature_implemented(nm, "ExposureTime"):
                nm.ExposureTime.value = min(25000, nm.ExposureTime.max)
            if enum_entry_implemented(nm, "Scan3dDataFilterSelector", "ValidationFilter"):
            ...
            print("Negotiated stream packet size: {}".format(
                nm.GevSCPSPacketSize.value))
            ...
            if nm.GevSCPSPacketSize.value <= 1500 and nm.GevSCPSPacketSize.max > 1500:
            ...
            # To demonstrate the acquisition loop, let's just fetch few buffers in a loop. Refer again to Harvester documentation
            ...
            print("\tNumber of blocks skipped (for any reason, camera or receiver side): {}".format(
                stream_nm.GevStreamSkippedBlockCount.value))
            print("\tNumber of discarded blocks (typically because they arrived corrupted with too big packet loss): {}".format(
                stream_nm.GevStreamDiscardedBlockCount.value))
            print("\tNumber of blocks that were delivered, but were missing some data packets: {}".format(
                stream_nm.GevStreamIncompleteBlockCount.value))
            print("\tNumber of blocks discarded by the acquisition if no acquisition buffers were free to fill at the moment: {}".format(
                stream_nm.GevStreamEngineUnderrunCount.value))
            print("\tNumber of all stream packets seen by the acquisition engine: {}".format(
                stream_nm.GevStreamSeenPacketCount.value))
            print("\tNumber of all packets detected as lost by the engine: {}".format(
                stream_nm.GevStreamLostPacketCount.value))
            print("\tNumber of all packets in successfully delivered frames: {}".format(
                stream_nm.GevStreamDeliveredPacketCount.value))
            print("\tNumber of packet resend request commands issued by the acquisition engine: {}".format(
                stream_nm.GevStreamResendCommandCount.value))
            print("\tNumber of all packets requested in those resend request commands: {}".format(
                stream_nm.GevStreamResendPacketCount.value))
            print("\tNumber of resend-requested packets which were marked as no more available by the camera: {}".format(
                stream_nm.GevStreamUnavailablePacketCount.value))
            ...
            with ia.fetch(timeout=2.0) as buffer:
                print(
                    "Acquired one more frame to demonstrate buffer contents processing")
                ...
                cmp_range = None
                cmp_intensity = None
                print("The buffer contains following {} components:".format(
                    len(buffer.payload.components)))
                for i, cmp in enumerate(buffer.payload.components):
                    print("\t#{}: size {}x{}, format {}".format(
                        i, cmp.width, cmp.height, cmp.data_format))
                    # Identify and remember the range & intensity components based on their pixel format as indicated above
                    # The Visionary by default uses BGR8 or Mono16 format for intensity (depending on the actual camera model
                    # you are testing with), Coord3D_C16 for range data
                    if cmp.data_format in ['BGR8', 'Mono16']:
                        cmp_intensity = cmp
                ...
                if not (cmp_range and cmp_intensity):
                    sys.exit(
                        "Failed to locate both Range and Intensity components in the acquired buffer")
                ...
                nm.ChunkScan3dCoordinateReferenceSelector.value = 'RotationX'
                ref_coord_rot_x = nm.ChunkScan3dCoordinateReferenceValue.value
                nm.ChunkScan3dCoordinateReferenceSelector.value = 'RotationY'
                ref_coord_rot_y = nm.ChunkScan3dCoordinateReferenceValue.value
                nm.ChunkScan3dCoordinateReferenceSelector.value = 'RotationZ'
                ref_coord_rot_z = nm.ChunkScan3dCoordinateReferenceValue.value
                nm.ChunkScan3dCoordinateReferenceSelector.value = 'TranslationX'
                ref_coord_trans_x = nm.ChunkScan3dCoordinateReferenceValue.value
                nm.ChunkScan3dCoordinateReferenceSelector.value = 'TranslationY'
                ref_coord_trans_y = nm.ChunkScan3dCoordinateReferenceValue.value
                nm.ChunkScan3dCoordinateReferenceSelector.value = 'TranslationZ'
                ref_coord_trans_z = nm.ChunkScan3dCoordinateReferenceValue.value
                # Print the values
                ...
                print("\tPrincipal point [U,V]: [{}, {}]".format(
                    princ_point_u, princ_point_v))
                ...
                print("\tOptional reference coordinate transformation parameters:")
                print("\t\tRotation X (in degrees): {}".format(ref_coord_rot_x))
                print("\t\tRotation Y (in degrees): {}".format(ref_coord_rot_y))
                print("\t\tRotation Z (in degrees): {}".format(ref_coord_rot_z))
                print("\t\tTranslation X (in mm): {}".format(ref_coord_trans_x))
                print("\t\tTranslation Y (in mm): {}".format(ref_coord_trans_y))
                print("\t\tTranslation Z (in mm): {}".format(ref_coord_trans_z))
                ...
                # The intensity component is known to be delivered (depending on camera model under test, see above)
                # - either in BGR8 format: 3 B-G-R channels, uint8 dtype,
                # - or in Mono16 format: 1 channel, uint16 dtype.
                # Before further processing (storing the image & point cloud below), let's first convert it to a common shape,
                # as a BGR image
                if cmp.data_format == 'BGR8':
                    # Let's reshape the array as image.
                    intensity_arr_bgr = cmp_intensity.data.reshape(
                        cmp_intensity.height, cmp_intensity.width, 3)
                else:
                    # The Mono16 case...
                    # When converting it to 8-bit format, scale the range for human eye in case the data is "too dark"
                    ...
                    intensity_arr_uint16 = cmp_intensity.data
                    max_intensity_val = numpy.amax(intensity_arr_uint16)
                    uint8_intensity_factor = max_intensity_val // 255 + 1
                    intensity_arr_uint8 = (
                        intensity_arr_uint16 // uint8_intensity_factor).astype(numpy.uint8)
                    # "Expand" the monochrome data to 3-channel format (repeating the monochrome value into all 3 channels)
                    # which is the format used in the code below
                    intensity_arr_bgr = numpy.repeat(intensity_arr_uint8, 3).reshape(
                        cmp_intensity.height, cmp_intensity.width, 3)

                # Reverse the channels to get RGB format
                # (Depending on whether further use of the data requires/benefits from a contiguous array, copy() might be needed or not
                ...
                intensity_image = Image.fromarray(
                    intensity_arr_rgb, mode="RGB")
                ...
                # Let's reshape the array as image.
                range_arr_uint16 = cmp_range.data.reshape(
                    cmp_range.height, cmp_range.width)
                ...
                range_arr_uint8 = (range_arr_uint16 //
                                   uint8_range_factor).astype(numpy.uint8)
                ...
                # Note: on current Visionary camera models the dimensions of the range and intensity components are either exactly equal,
                ...
                intensity_scale_x = cmp_intensity.width // cmp_range.width
                intensity_scale_y = cmp_intensity.height // cmp_range.height
                ...
                yc = yp * scaled_c
                zc = scaled_c
                # Note that the computed coordinates are in camera's native "Anchor" coordinate system which is related to its internal
                # geometry and might be affected by current operating mode and sensor mounting deviations. Compare this to the "Reference"
                # coordinate system with origin in the center of camera's front surface and Z pointing out of the camera. When conversion
                # to the Reference coordinate system is required, it can be performed here using the transformation parameters queried above,
                # in following order: 1. ref_coord_rot_x, 2. ref_coord_rot_y, 3. ref_coord_rot_z, 4. all translations.
                # Remember also that not all pixels might carry a valid measurement - the invalid ones might be marked using the corresponding
                ...
                # value parameters
                xcf = xc[not (invalid_flag_used)
                         or range_arr_uint16 != invalid_flag_value]
                ycf = yc[not (invalid_flag_used)
                         or range_arr_uint16 != invalid_flag_value]
                zcf = zc[not (invalid_flag_used)
                         or range_arr_uint16 != invalid_flag_value]
                # If we were going to use the color component as point cloud overlay, you might need to filter it similarly
                bgrf = bgr_scaled[not (invalid_flag_used)
                                  or range_arr_uint16 != invalid_flag_value]
...
def force_suitable_ip_address(device_info):
    print("Attempting to force suitable IP address into the (currently unreachable) device {}".format(
    ...
    for i in range(itf_node_map.DeviceSelector.max + 1):
        ...
        if device_id == itf_node_map.DeviceID.value:
            ...
            print("\tThe proposed IP address is {}, subnet mask {}, forcing it into the device".format(
                proposed_ip, proposed_subnet))
...
def store_ply_file(file, x, y, z, r, g, b):
    ...
    num_vertices = x.size
    vertices = numpy.empty(num_vertices, dtype=[(
...
# Trivial helper to check if GenICam feature with given name exists in the nodemap and is implemented by the currently connected
# device - note that because the same GenICam interface is used across multiple Visionary devices, some features that are listed
# in the nodemap might be implemented only by a subset of the Visionary camera models


def feature_implemented(nodemap, feature_name):
    return nodemap.has_node(feature_name) and genapi.is_implemented(nodemap.get_node(feature_name))

# Similar helper for individual enum entries within an enumerated feature


def enum_entry_implemented(nodemap, enum_name, entry_name):
    if not (feature_implemented(nodemap, enum_name)):
        return False
    enum = nodemap.get_node(enum_name)
    if not (hasattr(enum, "entries")):  # Shortcut to check if the node is an enumeration
        return False
    entry = enum.get_entry_by_name(entry_name)
    if (not (entry)):
        return False
    return genapi.is_implemented(entry)
...
# Enable output of version information
parser = argparse.ArgumentParser(
    description="Tutorial Harvesters script for SICK Visionary cameras")
