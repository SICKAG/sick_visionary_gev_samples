...
from common import init_harvester
...
def main(args):
    ...
    try:
        ...
        if args.serial_number:
            ...
            for idx, device in enumerate(h.device_info_list):
                if device.serial_number == args.serial_number:
                    selected_device_idx = idx
                    break
            if selected_device_idx is None:
                sys.exit("Could not find any discovered device with serial number '{}'".format(
                    args.serial_number))

        # Expected, that the selected device is the camera we want to use, adjust this if needed
        if h.device_info_list[selected_device_idx].access_status == DEVICE_ACCESS_STATUS_LIST.DEVICE_ACCESS_STATUS_NOACCESS:
            sys.exit(
                "No access to this device possible. Please check/review the ip settings of this device and retry.")

        # ----------------------------------
        # | Configuration and capture data |
        # ----------------------------------
        with h.create(selected_device_idx) as ia:
            ...
            with ia.fetch(timeout=2.0) as buffer:
                ...
                for i, cmp in enumerate(buffer.payload.components):
                    if cmp.data_format == 'Mono8':
                        ...
                        print("\tExpected number of IMU data samples: {}".format(
                            num_of_samples))
                        ...
                        print("\tExtracted IMU data samples from the buffer: {}".format(
                            len(imu_data)))
            ia.stop()
            ...
            # -----------------------------------------------
            print(
                '"id", "timestamp", "acc_x", "acc_y", "acc_z", "ang_x", "ang_y", "ang_z"')
            for i, sample in enumerate(imu_data):
                print("{:3d}, {}, {:9.6f}, {:9.6f}, {:9.6f}, {:9.6f}, {:9.6f}, {:9.6f} ".format(i, sample.timestamp,
                                                                                                sample.acceleration[0], sample.acceleration[
                                                                                                    1], sample.acceleration[2],
...
if __name__ == '__main__':
    parser = ArgumentParser(
        description="Capture one frame and print extracted IMU data samples")
    parser.add_argument('-s', '--serial_number', required=False,
                        help='Serial number of the device to use. If omitted, the first discovered device is used.')
